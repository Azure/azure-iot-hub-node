...
const fakeDevice = { deviceId: 'deviceId' };
...
const normalizedFakeSASDevice = {
  deviceId: 'deviceId',
  authentication: fakeAuthentication
};

const fakeConfig = { host: 'host', sharedAccessSignature: 'sas', x509: 'x509', tokenCredential: undefined };

const zeroDevices = [];
const oneHundredOneDevices = new Array(101).map(function (x, i) { return { deviceId: i.toString() }; });
...
function _testDevicesFalsyArg(methodUnderTest, argName, argValue, ExpectedErrorType) {
...
function testErrorCallback(methodUnderTest, arg1, arg2, arg3, arg4, arg5) {
  ...
  const invokeMethodUnderTest = function (registry, callback) {
    if (arg5) {
      registry[methodUnderTest](arg1, arg2, arg3, arg4, arg5, callback);
    } else if (arg4) {
      registry[methodUnderTest](arg1, arg2, arg3, arg4, callback);
  ...
  it('calls the done callback with a proper error if an HTTP error occurs', function (testCallback) {
    ...
    const registry = new Registry(fakeConfig, FakeHttpErrorHelper);
  ...
  it('calls the done callback with a standard error if not an HTTP error', function (testCallback) {
    ...
    const registry = new Registry(fakeConfig, FakeGenericErrorHelper);
    const callback = function (err, result, response) {
...
describe('Registry', function () {
  ...
  describe('#fromConnectionString', function () {
    ...
    it('returns a new instance of the Registry object', function () {
      const registry = Registry.fromConnectionString('HostName=a.b.c;SharedAccessKeyName=name;SharedAccessKey=key');
  ...
  describe('#fromSharedAccessSignature', function () {
    ...
    it('returns a new instance of the Registry object', function () {
      const registry = Registry.fromSharedAccessSignature('SharedAccessSignature sr=audience&sig=signature&se=expiry&skn=keyname');
      assert.instanceOf(registry, Registry);
    });
  });
  ...
  describe('#fromTokenCredential', function () {
    ...
    it('returns a new instance of the Registry object', function () {
      const registry = Registry.fromTokenCredential("hub.host.tv", fakeTokenCredential);
      assert.instanceOf(registry, Registry);
    });
  ...
  describe('#create', function () {
    ...
    it('constructs a valid HTTP request for a SAS device', function (testCallback) {
      const fakeHttpHelper = {
        executeApiCall: function (method, path, httpHeaders, body, done) {
          ...
          assert.equal(path, '/devices/' + fakeDevice.deviceId + versionQueryString());
    ...
    it('defaults to SAS authentication when the device does not have at least one x509 thumbprint', function (testCallback) {
      ...
      const registry = new Registry(fakeConfig, fakeHttpHelper);
      const fakeDevice = new Device(null);
  ...
  describe('#list', function () {
    ...
    it('constructs a valid HTTP request', function (testCallback) {
      const fakeHttpHelper = {
        executeApiCall: function (method, path, httpHeaders, body, done) {
          assert.equal(method, 'GET');
          assert.equal(path, '/devices/' + versionQueryString());
          done();
        }
      };
  ...
  describe('#getTwin', function () {
    ...
    //
    // I don't quite understand this test and it hangs.
    //
    ...
    //     const registry = new Registry({ host: 'host', sharedAccessSignature: 'sas' });
    //     let promise = registry.getTwin('deviceId', badCallback);
    ...
    /*Tests_SRS_NODE_IOTHUB_REGISTRY_16_036: [The `getTwin` method shall call the `done` callback with a `Twin` object updated with the latest property values stored in the IoT Hub service.]*/
  ...
  describe('#updateTwin', function () {
    ...
    it('creates a valid HTTP request', function (testCallback) {
      ...
      const fakeHttpResponse = { statusCode: 200 };
      const fakeEtag = 'etag==';

      const fakeHttpHelper = {
        executeApiCall: function (method, path, httpHeaders, body, done) {
          assert.equal(method, 'PATCH');
          assert.equal(path, '/twins/' + fakeDeviceId + versionQueryString());
          assert.equal(httpHeaders['Content-Type'], 'application/json; charset=utf-8');
  ...
  describe('#importDevicesFromBlob', function () {
    ...
    ```]*/
    it('constructs a valid HTTP request', function (testCallback) {
      const fakeInputBlob = "input";
      const fakeOutputBlob = "output";
      const fakeHttpHelper = {
        executeApiCall: function (method, path, httpHeaders, body, done) {
          assert.equal(method, 'POST');
          assert.equal(path, '/jobs/create' + versionQueryString());
          assert.equal(httpHeaders['Content-Type'], 'application/json; charset=utf-8');
  ...
  describe('#importDevicesFromBlobByIdentity', function () {
    ...
    it('constructs a valid HTTP request', function (testCallback) {
      const fakeInputBlob = "input";
      const fakeOutputBlob = "output";
      const fakeUserAssignedIdentity = "identity";
      const fakeHttpHelper = {
        executeApiCall: function (method, path, httpHeaders, body, done) {
          assert.equal(method, 'POST');
          assert.equal(path, '/jobs/create' + versionQueryString());
  ...
  describe('#listJobs', function () {
    ...
    it('constructs a valid HTTP request', function (testCallback) {
      const fakeHttpHelper = {
        executeApiCall: function (method, path, httpHeaders, body, done) {
          assert.equal(method, 'GET');
          assert.equal(path, '/jobs' + versionQueryString());
          done();
        }
      };
  ...
  describe('#getJob', function () {
    ...
    it('constructs a valid HTTP request', function (testCallback) {
      ...
      const fakeHttpHelper = {
        executeApiCall: function (method, path, httpHeaders, body, done) {
          assert.equal(method, 'GET');
          assert.equal(path, '/jobs/' + fakeJob.jobId + versionQueryString());
          done();
        }
      };
  ...
  describe('#cancelJob', function () {
    ...
    it('constructs a valid HTTP request', function (testCallback) {
      ...
      const fakeHttpHelper = {
        executeApiCall: function (method, path, httpHeaders, body, done) {
          assert.equal(method, 'DELETE');
          assert.equal(path, '/jobs/' + fakeJobId + versionQueryString());
          done();
        }
      };
  ...
  describe('#createQuery', function () {
    ...
    it('returns a Query instance', function () {
      const registry = new Registry(fakeConfig, {});
      const query = registry.createQuery('SELECT * FROM devices', 42);
  ...
  describe('#_executeQueryFunc', function () {
    ...
    [{
      ...
      it('constructs a valid HTTP request with pageSize: \'' + testConfig.fakePageSize + '\' and continuation token: \'' + testConfig.fakeContinuationToken + '\'', function () {
        const fakeSql = 'sql';
        const fakePageSize = testConfig.fakePageSize;
        const fakeQuery = {
          query: fakeSql
        };

        const fakeHttpHelper = { executeApiCall: sinon.stub().callsArgWith(4, null, [], { statusCode: 200, headers: { 'x-ms-continuation': testConfig.fakeContinuationToken } }) };

        const registry = new Registry(fakeConfig, fakeHttpHelper);
        const query = registry.createQuery(fakeSql, fakePageSize);
        query.next(function () {
          query.next(function () {});
        });
        assert.strictEqual(fakeHttpHelper.executeApiCall.args[0][0], 'POST');
        assert.strictEqual(fakeHttpHelper.executeApiCall.args[0][1], '/devices/query' + versionQueryString());
  ...
  describe('#getRegistryStatistics', function () {
    ...
    it('constructs a valid HTTP request', function (testCallback) {
      const fakeHttpHelper = {
        executeApiCall: function (method, path, httpHeaders, body, done) {
          assert.equal(method, 'GET');
          assert.equal(path, '/statistics/devices' + versionQueryString());
          done();
        }
      };
    ...
    describe('#updateDevices', function () {
      ...
      [{ force: true, importMode: 'Update' }, { force: false, importMode: 'UpdateIfMatchETag' }].forEach(function (forceArg) {
        it('constructs a valid HTTP request with forceUpdate of ' + forceArg.force + ' and importMode of ' + forceArg.importMode, function (testCallback) {
          const updatedDevices = [{ deviceId: 'deviceone' },{ deviceId: 'devicetwo' }];
          const sentBody = [
    ...
    describe('#removeDevices', function () {
      ...
      [{ force: true, importMode: 'Delete' }, { force: false, importMode: 'DeleteIfMatchETag' }].forEach(function (forceArg) {
        it('constructs a valid HTTP request with forceDelete of ' + forceArg.force + ' and importMode of ' + forceArg.importMode, function (testCallback) {
          const removedDevices = [{ deviceId: 'deviceone' },{ deviceId: 'devicetwo' }];
          const sentBody = [
            { id:'deviceone', importMode: forceArg.importMode, authentication: { type: 'sas', symmetricKey: { primaryKey: '', secondaryKey: '' } } },
            { id:'devicetwo', importMode: forceArg.importMode, authentication: { type: 'sas', symmetricKey: { primaryKey: '', secondaryKey: '' } } }
          ];
    ...
    it('constructs a valid HTTP request', function (testCallback) {
      const fakeHttpHelper = {
        executeApiCall: function (method, path, httpHeaders, body, done) {
          ...
          assert.strictEqual(path, '/twins/deviceId/modules/moduleId' + versionQueryString());
    ...
    it('constructs a valid HTTP request', function (testCallback) {
      const fakeHttpHelper = {
        executeApiCall: function (method, path, httpHeaders, body, done) {
          ...
          assert.strictEqual(path, '/configurations/configId' + versionQueryString());
          assert.strictEqual(httpHeaders['Content-Type'], 'application/json; charset=utf-8');
          const fakeConfigCopy = JSON.parse(JSON.stringify(fakeConfiguration));
  ...
  describe('#getConfigurations', function () {
    ...
    it('constructs a valid HTTP request', function (testCallback) {
      const fakeHttpHelper = {
        executeApiCall: function (method, path, httpHeaders, body, done) {
          assert.strictEqual(method, 'GET');
          assert.strictEqual(path, '/configurations' + versionQueryString());
          done();
        }
      };
    ...
    it('constructs a valid HTTP request', function (testCallback) {
      ...
      const fakeHttpHelper = {
        executeApiCall: function (method, path, httpHeaders, body, done) {
          ...
          assert.strictEqual(path, '/devices/deviceId/applyConfigurationContent' + versionQueryString());
          assert.strictEqual(httpHeaders['Content-Type'], 'application/json; charset=utf-8');
  ...
  describe('#addModule', function () {
    ...
    it('constructs a valid HTTP request', function (testCallback) {
      const fakeHttpHelper = {
        executeApiCall: function (method, path, httpHeaders, body, done) {
          assert.strictEqual(method, 'PUT');
          assert.strictEqual(path, '/devices/deviceId/modules/moduleId' + versionQueryString());
          assert.strictEqual(httpHeaders['Content-Type'], 'application/json; charset=utf-8');
          const moduleCopy = JSON.parse(JSON.stringify(fakeModule));
  ...
  describe('#getModulesOnDevice', function () {
    ...
    it('constructs a valid HTTP request', function (testCallback) {
      const fakeHttpHelper = {
        executeApiCall: function (method, path, httpHeaders, body, done) {
          assert.strictEqual(method, 'GET');
          assert.strictEqual(path, '/devices/deviceId/modules' + versionQueryString());
          done();
        }
      };
  ...
  describe('#removeModule', function () {
    ...
    it('constructs a valid HTTP request with a module object', function (testCallback) {
      const fakeHttpHelper = {
        executeApiCall: function (method, path, httpHeaders, body, done) {
          assert.strictEqual(method, 'DELETE');
          assert.strictEqual(path, '/devices/device-from-object/modules/module-from-object' + versionQueryString());
